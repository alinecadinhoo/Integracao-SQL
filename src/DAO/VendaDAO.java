...
public class VendaDAO extends GenericDAO<Venda, String> implements IVendaDAO {
    ...
    public void finalizarVenda(Venda venda) throws TipoChaveNaoEncontradaException, DAOException {
        ...
        try {
            ...
            atualizarEstoqueProdutosVendidos(connection, venda);
    ...
    private void atualizarEstoqueProdutosVendidos(Connection connection, Venda venda) throws DAOException {
        PreparedStatement stmEstoque = null;
        try {
            StringBuilder sbEstoque = new StringBuilder();
            sbEstoque.append("UPDATE TB_ESTOQUE SET QUANTIDADE = QUANTIDADE - ? ");
            sbEstoque.append("WHERE ID_PRODUTO_FK = ?");
            stmEstoque = connection.prepareStatement(sbEstoque.toString());

            for (ProdutoQuantidade prod : venda.getProdutos()) {
                stmEstoque.setInt(1, prod.getQuantidade());
                stmEstoque.setLong(2, prod.getProduto().getId());
                stmEstoque.addBatch();
            }

            stmEstoque.executeBatch();
        } catch (SQLException e) {
            throw new DAOException("ERRO ATUALIZANDO ESTOQUE ", e);
        } finally {
            closeStatement(stmEstoque);
        }
    }

    private void closeStatement(PreparedStatement stmEstoque) {
    }
    ...
    @Override
    public void setParametrosQueryExclusao(PreparedStatement stmInsert, String valor) throws SQLException {
        throw new UnsupportedOperationException("OPERAÇÃO NÃO PERMITIDA");
    }
    ...
    @Override
    public void setParametrosQuerySelect(PreparedStatement stm, String valor) throws SQLException {
